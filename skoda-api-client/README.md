...
Flash your vehicle's lights. The API server retrieves the vehicle's current reported position and supplies it to MySkoda as required by the command.
```java
VehicleService vehicleService = new VehicleService("YOUR_EMAIL", "YOUR_PASSWORD");
vehicleService.flashVehicleLights("YOUR_CARS_VIN");
```

## Honk and Flash
Honk the horn and flash the vehicle's lights. MySkoda exposes these as a combined operation rather than a horn-only operation.
```java
VehicleService vehicleService = new VehicleService("YOUR_EMAIL", "YOUR_PASSWORD");
vehicleService.honkAndFlashVehicle("YOUR_CARS_VIN");
```

## Lock Vehicle
Lock the vehicle using your four-digit MySkoda security PIN (S-PIN). Keep the S-PIN in a secret store rather than source code or logs.
```java
VehicleService vehicleService = new VehicleService("YOUR_EMAIL", "YOUR_PASSWORD");
vehicleService.lockVehicle("YOUR_CARS_VIN", System.getenv("SKODA_SPIN"));
```

## Unlock Vehicle
Unlock the vehicle using your four-digit MySkoda security PIN (S-PIN). Treat this operation as security-sensitive and require explicit confirmation when exposing it through automation.
```java
VehicleService vehicleService = new VehicleService("YOUR_EMAIL", "YOUR_PASSWORD");
vehicleService.unlockVehicle("YOUR_CARS_VIN", System.getenv("SKODA_SPIN"));
```
